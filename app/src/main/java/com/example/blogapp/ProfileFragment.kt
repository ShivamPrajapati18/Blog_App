...
class ProfileFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Glide.with(requireParentFragment()).load(img).centerCrop()
            .placeholder(R.drawable.baseline_person_24).into(binding.profileImage)

        mAdapter = PostAdapter(
            ::onClickReadButton, mAuth.uid, ::onDelete, ::onUpdateButton, ::onClickLikedButton
        )
    ...
    private fun setUpAdapter(item: List<Post>) {
        if (item.isNotEmpty()) {
            binding.userAllPost.adapter = mAdapter
